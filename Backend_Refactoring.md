Comprehensive Implementation Plan for FluentPro Backend Refactoring
  ...
  ## Goal
  Complete the domain-driven design structure by organizing all code into proper domain boundaries with clear separation of concerns.

  ## Current State Before This Step
  - ✅ Use cases have been created in domains/authentication/use_cases/ and domains/onboarding/use_cases/
  - ✅ Service interfaces exist in domains/*/services/interfaces.py
  - ❌ Models still exist in old structure (authentication/models/, onboarding/models/)
  - ❌ Domain subdirectories (dto/, events/, tasks/, api/, repositories/) are missing
  - ❌ Old app structure (authentication/, onboarding/) still contains business logic and views

  ## Step 1: Create Complete Domain Directory Structure ✅
  
  **Actions:** ✅
  1. Create missing subdirectories for each domain:
  ```bash
  # For authentication domain
  mkdir -p domains/authentication/models ✅
  mkdir -p domains/authentication/repositories ✅
  mkdir -p domains/authentication/dto ✅
  mkdir -p domains/authentication/events ✅
  mkdir -p domains/authentication/tasks ✅
  mkdir -p domains/authentication/api/v1 ✅
  
  # For onboarding domain
  mkdir -p domains/onboarding/models ✅
  mkdir -p domains/onboarding/repositories ✅
  mkdir -p domains/onboarding/dto ✅
  mkdir -p domains/onboarding/events ✅
  mkdir -p domains/onboarding/tasks ✅
  mkdir -p domains/onboarding/api/v1 ✅
  
  # For shared domain
  mkdir -p domains/shared/models ✅
  mkdir -p domains/shared/value_objects ✅
  ```

  2. Create __init__.py files in all new directories:
  ```bash
  touch domains/authentication/{models,repositories,dto,events,tasks}/__init__.py ✅
  touch domains/authentication/api/__init__.py ✅
  touch domains/authentication/api/v1/__init__.py ✅
  # Repeat for onboarding and shared domains ✅
  ```

  **Verification:** ✅
  - Run `ls -la domains/authentication/` and verify all subdirectories exist ✅
  - Run `ls -la domains/onboarding/` and verify all subdirectories exist ✅

  ## Step 2: Move and Refactor Models to Domain Structure 

  **Actions:** 
  1. Move authentication models: ✅
  ```bash
  # Move and rename files 
  mv authentication/models/user.py domains/authentication/models/user.py ✅
  mv authentication/models/role.py domains/authentication/models/role.py ✅
  mv authentication/models/auth.py domains/authentication/models/auth_token.py ✅
  ```

  2. Update imports in moved files:
  ```python
  # In domains/authentication/models/user.py
  # Change: from authentication.models import BaseModel
  # To: from domains.shared.models.base_entity import BaseEntity ✅
  ```

  3. Move onboarding models:
  ```bash
  # Move and rename files
  mv onboarding/models/onboarding.py domains/onboarding/models/onboarding_session.py ✅
  mv onboarding/models/communication.py domains/onboarding/models/communication_partner.py ✅
  
  # Create additional model files
  touch domains/onboarding/models/industry.py ✅
  touch domains/onboarding/models/language_proficiency.py ✅
  ```
 
  4. Update model class names to match new filenames: ✅
  ```python
  # In domains/onboarding/models/onboarding_session.py
  # Change: class Onboarding:
  # To: class OnboardingSession:
  ```

  **Verification:**
  - Models no longer exist in `authentication/models/` or `onboarding/models/` ✅
  - All model files are in `domains/*/models/` ✅
  - Run tests to ensure imports are correctly updated ✅

  ## Step 3: Create Repository Interfaces and Move Repository Implementations
  ...
  **Actions:**
  ...
  2. Move existing repositories from shared/repositories/:
  ```bash
  mv shared/repositories/user_repository.py domains/authentication/repositories/user_repository.py
  mv shared/repositories/role_repository.py domains/authentication/repositories/role_repository.py
  mv shared/repositories/communication_repository.py domains/onboarding/repositories/partner_repository.py
  mv shared/repositories/industry_repository.py domains/onboarding/repositories/industry_repository.py
  ```

  3. Update repository imports and interfaces:
  ```python
  # In domains/authentication/repositories/user_repository.py
  ...
  **Verification:**
  - No repositories remain in `shared/repositories/`
  - Each domain has its own repositories with corresponding interfaces
  - Repository implementations inherit from their interfaces

  ## Step 4: Create DTOs for Each Domain
  ...
  **Actions:**
  ...
  2. Create onboarding DTOs:
  ```python
  # domains/onboarding/dto/requests.py
  ...
  **Verification:**
  - Each domain has `dto/requests.py` and `dto/responses.py`
  - DTOs use Pydantic for validation
  - Mappers exist to convert between models and DTOs
  ...
  **Actions:**
  ...
  3. Create specification pattern for complex queries:
  ```python
  ...
  **Actions:**
  ...
  2. Create shared base repository implementation:
  ```python
