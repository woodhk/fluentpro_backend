...
async def test_state_inspector_functionality():
    ...
    try:
        ...
        conversation1.add_assistant_message("I'd be happy to help! What specific Python topic?")
        ...
        conversation2.add_assistant_message("Of course! What kind of debugging help do you need?")
        
        # Store conversations using state manager
        await inspector.state_manager.create_conversation(
            conversation1.user_id,
            context=conversation1.context,
            ttl_seconds=300
        )
        ...
        try:
            retrieved_conv = await inspector.state_manager.get_conversation(conversation1.conversation_id)
            if retrieved_conv:
        ...
        try:
            ...
            print(f"✅ Created problematic conversation with {len(problematic_conv.messages)} messages")
...
async def test_health_monitoring():
    ...
    try:
        ...
        if "metrics" in report and report["metrics"]:
            print(f"✅ Metrics report generated with {len(report['metrics'])} metric types")
...
async def test_state_visualization():
    ...
    try:
        ...
        print(f"   - Conversation Settings: {len(ai_context.get('conversation_settings', {}))}")
        ...
        print(f"   - Usage: {(conversation.total_tokens_used/conversation.token_limit)*100:.1f}%")
...
async def test_debugging_capabilities():
    ...
    try:
        ...
        print(f"✅ Created large conversation: {len(large_conversation.messages)} messages")
        ...
        token_heavy_conversation.add_user_message("This conversation uses too many tokens")
        
        print(f"✅ Created token-heavy conversation: {token_heavy_conversation.total_tokens_used} tokens")
        ...
        inconsistent_conversation.total_tokens_used = 200  # Doesn't match message tokens
        ...
        high_token_convs = [conv for conv in all_conversations if conv.total_tokens_used > 1000]
