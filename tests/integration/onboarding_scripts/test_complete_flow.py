...
from authentication.models.role import HierarchyLevel, JobDescription
    ...
    try:
        ...
        try:
            ...
            for i, match in enumerate(financial_matches, 1):
                print(
                    f"      {i}. {match.role.title} (Score: {match.relevance_score:.3f})"
                )
        ...
        if "financial_matches" in locals() and financial_matches:
            ...
            print(
                f"   Selecting best match: {best_match.role.title} (ID: {best_match.role.id})"
            )

            try:
                selection_result = user_manager.update_selected_role(
                    auth0_id, best_match.role.id
                )

                if selection_result.get(
                    "success", True
                ):  # Some methods return dict, some return the object directly
                    print(f"   ✅ Role selection successful!")
                    print(
                        f"      Selected Role: {selection_result.get('role_title', best_match.role.title)}"
                    )
                    ...
                    print(
                        f"   ❌ Role selection failed: {selection_result.get('error')}"
                    )
        ...
        try:
            ...
            if not custom_role_found:
                print(
                    f"   ⚠️  Custom role not found in search results (may take a moment to index)"
                )
