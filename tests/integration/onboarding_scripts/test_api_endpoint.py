...
from domains.authentication.api.v1.role_views import JobInputView
...
def test_job_input_endpoint():
    ...
    try:
        ...
        # Create request
        request = factory.post("/api/v1/role/job-input/", data=test_data, format="json")
        ...
        if response.status_code == 200:
            ...
            if "user_job_input" in response_data:
                ...
                print(
                    f"      Description: {job_input.get('job_description', '')[:80]}..."
                )
            ...
            if "matched_roles" in response_data:
                ...
                for i, role in enumerate(matched_roles, 1):
                    print(
                        f"      {i}. {role.get('title')} (Score: {role.get('relevance_score', 0):.3f})"
                    )
                    print(f"         Industry: {role.get('industry_name')}")
                    print(f"         ID: {role.get('id')}")
                    print(
                        f"         Description: {role.get('description', '')[:80]}..."
                    )
        ...
        hotel_request = factory.post(
            "/api/v1/role/job-input/", data=hotel_test_data, format="json"
        )
