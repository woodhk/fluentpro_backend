...
async def test_distributed_locks_basic():
    ...
    try:
        ...
        async with distributed_lock(
            lock_impl, "test_lock_2", ttl_seconds=5
        ) as lock_mgr:
...
async def test_concurrent_state_updates():
    ...
    try:
        ...
        context_manager = ConversationStateManagerFactory.create_context_manager(
            state_manager
        )
        ...
        conversation = await state_manager.create_conversation(
            user_id=user_id, context=context, ttl_seconds=300
        )
        ...
        async def update_with_lock(worker_id: int):
            ...
            for i in range(5):
                ...
                try:
                    async with distributed_lock(
                        lock_impl, lock_key, ttl_seconds=10, timeout_seconds=5
                    ) as lock_mgr:
                        # Get current conversation (within lock)
                        current_conv = await state_manager.get_conversation(
                            conversation_id
                        )
                        if not current_conv:
                            print(f"❌ Worker {worker_id}: Conversation not found")
                        ...
                        message_content = (
                            f"Locked message from worker {worker_id}, iteration {i}"
                        )
                        result = await context_manager.add_user_message(
                            conversation_id=conversation_id,
                            content=message_content,
                        ...
                        if result:
                            ...
                            print(
                                f"❌ Worker {worker_id}: Failed to add locked message {i}"
                            )

                except (LockAcquisitionError, LockTimeoutError) as e:
                    print(
                        f"⏰ Worker {worker_id}: Lock timeout/error on iteration {i}: {e}"
                    )
        ...
        messages_with_locks = (
            len(final_conv_locked.messages) if final_conv_locked else 0
        )
    ...
    try:
        ...
        for i in range(100):
            try:
                ...
                async with distributed_lock(
                    lock_impl, lock_key, ttl_seconds=1
                ) as lock_mgr:
