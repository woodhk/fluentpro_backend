...
class MockPasswordService(IPasswordService):
    ...
    async def analyze_password_security_async(self, password: str, context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        return {
            ...
            "recommendations": ["Use more special characters", "Avoid sequential patterns"],
    ...
    async def generate_contextual_password_async(self, user_context: Dict[str, Any], length: int = 16) -> Dict[str, Any]:
        ...
        return {
            ...
            "security_analysis": {"score": 90, "strengths": ["good entropy", "mixed characters"]},
...
class MockRecommendationService(IRecommendationService):
    ...
    def get_course_recommendations(self, user_id: str, role_id: Optional[str] = None, limit: int = 10) -> List[Dict[str, Any]]:
        return [
    ...
    def record_recommendation_feedback(self, user_id: str, recommendation_id: str, feedback_type: str, metadata: Optional[Dict[str, Any]] = None) -> bool:
        return True
    
    def get_recommendation_history(self, user_id: str, recommendation_type: Optional[str] = None, limit: int = 50) -> List[Dict[str, Any]]:
        return [
            {"recommendation_id": "rec_1", "type": "role", "feedback": "accepted"}
        ]
    
    # Async AI-powered methods (new functionality)
    async def get_ai_role_recommendations_async(self, user_id: str, user_input: str, context: Optional[Dict[str, Any]] = None, limit: int = 10) -> Dict[str, Any]:
        await asyncio.sleep(0.1)
        return {
            "recommendations": [
                {"role_id": "ai_role_1", "title": "AI Engineer", "ai_score": 0.95, "reasoning": "Strong match for technical skills"}
            ],
    ...
    async def get_dynamic_course_recommendations_async(self, user_id: str, learning_goals: List[str], current_progress: Optional[Dict[str, Any]] = None, limit: int = 10):
        await asyncio.sleep(0.1)
        for i in range(3):
            yield {
                "course_recommendation": {"course_id": f"dynamic_course_{i}", "title": f"AI Course {i+1}"},
    ...
    async def generate_personalized_learning_path_async(self, user_id: str, target_role: str, timeline: Optional[int] = None) -> Dict[str, Any]:
        await asyncio.sleep(0.1)
        return {
            "learning_path": [
            ...
            "milestone_roadmap": ["Complete basics", "Build portfolio", "Apply to roles"],
    ...
    async def analyze_communication_needs_async(self, user_id: str, role_context: Dict[str, Any]) -> Dict[str, Any]:
        await asyncio.sleep(0.1)
        return {
            "communication_analysis": {"primary_needs": ["client meetings", "team collaboration"]},
            "partner_recommendations": [{"partner": "business_client", "priority": "high"}],
            "unit_recommendations": [{"unit": "project_discussion", "difficulty": "medium"}],
            "scenario_suggestions": ["quarterly review meeting", "project planning session"],
            "difficulty_progression": ["beginner", "intermediate", "advanced"]
        }
    
    async def predict_learning_outcomes_async(self, user_id: str, proposed_path: Dict[str, Any]) -> Dict[str, Any]:
        await asyncio.sleep(0.1)
        return {
    ...
    async def generate_adaptive_recommendations_async(self, user_id: str, interaction_data: Dict[str, Any]):
        await asyncio.sleep(0.1)
        for i in range(2):
            yield {
...
class MockOnboardingService(IOnboardingService):
    ...
    async def start_adaptive_onboarding_async(self, user_id: str, user_context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "personalized_steps": ["ai_profile_analysis", "smart_recommendations", "adaptive_goals"],
...
async def test_password_service_dual_compatibility():
    ...
    print(f"  analyze_password_security_async: score={security_analysis['security_score']}")
...
async def test_session_service_dual_compatibility():
    ...
    print(f"  analyze_user_session_patterns_async: confidence={patterns['pattern_confidence']}")
...
async def test_recommendation_service_dual_compatibility():
    ...
    # Test async AI methods (new functionality)
    print("✅ Async AI methods:")
    ai_roles = await service.get_ai_role_recommendations_async("user_123", "I'm a software engineer looking for AI roles")
    print(f"  get_ai_role_recommendations_async: {len(ai_roles['recommendations'])} AI-recommended roles")
    
    learning_path = await service.generate_personalized_learning_path_async("user_123", "AI Engineer")
    print(f"  generate_personalized_learning_path_async: {len(learning_path['learning_path'])} phases")
    
    print("✅ Async streaming methods:")
    course_count = 0
    async for course in service.get_dynamic_course_recommendations_async("user_123", ["AI", "ML"]):
        course_count += 1
        print(f"  dynamic course {course_count}: {course['course_recommendation']['title']}")
...
async def test_onboarding_service_dual_compatibility():
    ...
    # Test async AI methods (new functionality)
    print("✅ Async AI methods:")
    adaptive = await service.start_adaptive_onboarding_async("user_123", {"role": "engineer"})
    ...
    print(f"  predict_onboarding_success_async: {prediction['success_probability']}% success probability")
