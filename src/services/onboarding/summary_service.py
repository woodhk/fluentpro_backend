...
class OnboardingSummaryService:
    ...
    async def get_onboarding_summary(self, auth0_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            summary = {
                ...
                "onboarding_status": user.get("onboarding_status", OnboardingStatus.PENDING.value),
                "is_complete": user.get("onboarding_status") == OnboardingStatus.COMPLETED.value
            }
    ...
    async def _get_industry_details(self, industry_id: Optional[str]) -> Optional[Dict[str, Any]]:
        """Get industry details by ID."""
        if not industry_id:
            return None
            
        try:
            result = self.db.table("industries")\
                .select("id, name")\
                .eq("id", industry_id)\
                .execute()
            ...
            for partner in partners:
                ...
                if partner.get("communication_partners"):
                    partner_summaries.append({
                        ...
                        "description": partner["communication_partners"].get("description"),
