...
class JobMatchingService:
    ...
    async def select_role(
        ...
        try:
            ...
            if role_id:
                ...
                custom_description = sanitize_string(custom_description, max_length=1000)
                
                # Generate embedding for custom role
                combined_text = f"Job Title: {custom_title}\n\nDescription: {custom_description}"
                embedding = await openai_client.generate_embedding(combined_text)
                ...
                raise ValueError("Either role_id or custom role details must be provided")
    ...
    async def _index_single_role(self, role: Dict[str, Any], industry_id: str):
        ...
        try:
            # Get industry name
            industry = await self.profile_repo.db.table("industries").select("name").eq("id", industry_id).execute()
